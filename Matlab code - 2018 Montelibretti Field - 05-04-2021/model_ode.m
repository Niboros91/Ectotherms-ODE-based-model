...
for t=1:(Simulation_time-1) %Loop for the simulation
    ...
    %We recompute A based on the new temperature
...
figure
plot(t,x_hist(1,:),t,x_hist(2,:),t,x_hist(3,:),t,x_hist(4,:),t,x_hist(5,:),t,x_hist(6,:),...
t,x_hist(7,:),t,x_hist(8,:),'LineWidth',2);
legend("Egg","Larva 1","Larva 2","Larva 3","Pupa","Adult male", "Un-mated female", "Mated female",'Fontsize',15);
legend boxoff  
xlabel('Time [days]','Fontsize',15);
ylabel('Number insects','Fontsize',15);
%xlim([t(0) t(260)])
ylim([0 1100])
title('Montelibretti Field 2018','Fontsize',20);
set(gca,'FontSize',15)
...
T = table(transpose(tt), transpose(x_hist))
%T = table(transpose(tt), transpose(x_hist(1,:)), transpose(x_hist(2,:)), transpose(x_hist(3,:)), transpose(x_hist(4,:)), transpose(x_hist(5,:)), transpose(x_hist(6,:)), transpose(x_hist(7,:)), transpose(x_hist(8,:)));
writetable(T, 'Allstages.csv', 'Delimiter', ';');
